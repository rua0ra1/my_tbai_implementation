...
WbcBase::WbcBase(const std::string &configFile, const std::string &urdfString,
    ...
    nGeneralizedCoordinates_ = 6 + JOINT_COORDINATE_SIZE;

    // Base angular + linear acceleration, joint accelerations, contact forces
    nDecisionVariables_ = nGeneralizedCoordinates_ + 3 * NUM_CONTACT_POINTS;
...
Task WbcBase::createDynamicsTask() {
    ...
    const matrix_t Jcontact_base = Jcontact_.block(0, 0, NUM_CONTACT_POINTS * 3, 6);
...
Task WbcBase::createContactForceTask() {
    ...
    const size_t Arows = 3 * (NUM_CONTACT_POINTS - nContacts_);
...
Task WbcBase::createTorqueLimitTask() {
    ...
    const matrix_t &Mj = data.M.block<JOINT_COORDINATE_SIZE, 18>(6, 0);
    const vector_t &hj = data.nle.segment<JOINT_COORDINATE_SIZE>(6);
    const matrix_t &JjT = Jcontact_.block(0, 6, 12, JOINT_COORDINATE_SIZE).transpose();

    const size_t Drows = 2 * JOINT_COORDINATE_SIZE;
    matrix_t D = matrix_t::Zero(Drows, nDecisionVariables_);

    // upper bound
    D.block(0, 0, JOINT_COORDINATE_SIZE, nDecisionVariables_) << Mj, -JjT;

    // lower bound
    D.block(JOINT_COORDINATE_SIZE, 0, JOINT_COORDINATE_SIZE, nDecisionVariables_) << -Mj, JjT;

    vector_t f = vector_t::Ones(Drows);

    // upper bound
    f.head<JOINT_COORDINATE_SIZE>() *= torqueLimit_;
    f.head<JOINT_COORDINATE_SIZE>() -= hj;

    // lower bound
    f.tail<JOINT_COORDINATE_SIZE>() *= torqueLimit_;
...
Task WbcBase::createBaseAccelerationTask(const vector_t &stateCurrent, const vector_t &stateDesired,
    ...
    const vector_t &jointPositions = stateDesired.tail<JOINT_COORDINATE_SIZE>();
    const vector_t &jointVelocities = inputDesired.tail<JOINT_COORDINATE_SIZE>();
    ...
    auto eulerError = rotationErrorInLocalEulerXYZ(eulerCurrent, eulerDesired);
...
Task WbcBase::createSwingFootAccelerationTask(const vector_t &stateCurrent, const vector_t &inputCurrent,
    ...
    const vector_t &jointPositionsDesired = stateDesired.tail<JOINT_COORDINATE_SIZE>();
    const vector_t &jointVelocitiesDesired = inputDesired.tail<JOINT_COORDINATE_SIZE>();
    ...
    const vector_t &jointPositionsCurrent = stateCurrent.tail<JOINT_COORDINATE_SIZE>();
    const vector_t &jointVelocitiesCurrent = inputCurrent.tail<JOINT_COORDINATE_SIZE>();
...
void WbcBase::updateMeasuredState(const vector_t &stateMeasured, const vector_t &inputMeasured) {
    ...
    qMeasured_.tail<JOINT_COORDINATE_SIZE>() = stateMeasured.tail<JOINT_COORDINATE_SIZE>();
    ...
    rMeasured_ = rotationMatrixBaseToOrigin(eulerXYZ);
    ...
    vMeasured_.tail<JOINT_COORDINATE_SIZE>() = inputMeasured.tail<JOINT_COORDINATE_SIZE>();
...
void WbcBase::updateKinematicsAndDynamicsDesired(const vector_t &stateDesired, const vector_t &inputDesired) {
    ...
    rDesired_ = rotationMatrixBaseToOrigin(eulerXYZ);
}
